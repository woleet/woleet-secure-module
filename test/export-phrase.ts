...
describe('phrase export', () => {
  ...
  it('exportPhrase function should reject if called without argument', async () => {
    // @ts-ignore
    await assert.rejects(() => sm.exportPhrase());
  });

  it('exportPhrase function should be callable with one valid arguments', async () => {
    assert.doesNotThrow(() => sm.exportPhrase(encryptedKey.entropy));
    await assert.doesNotReject(() => sm.exportPhrase(encryptedKey.entropy));
  });

  it('exportPhrase function should reject with one invalid argument', async () => {
    // @ts-ignore
    await assert.rejects(() => sm.exportPhrase('test'));
  });

  it('exportPhrase function should reject with two arguments', async () => {
    // @ts-ignore
    await assert.rejects(() => sm.exportPhrase(encryptedKey.entropy, 'test'));
  });

  it('exportPhrase function should reject with one invalid arguments (1)', async () => {
    const invalid = Buffer.from(encryptedKey.entropy);
    crypto.randomBytes(5).copy(invalid);
    await assert.rejects(() => sm.exportPhrase(invalid));
  });

  it('exportPhrase function should reject with one invalid arguments (2)', async () => {
